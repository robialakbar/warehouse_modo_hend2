...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card-header">
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add-user" onclick="addUser()"><i class="fa fa-plus"></i> Add New User</button>
        </div>
        <div class="card-body">
            ...
            @if(count($users) > 0)
                ...
                @php
                    $data = ["user_id" => $d->id, "fullname" => $d->name, "username" => $d->username, "role" => $d->role, "city_id" => $d->city_id, "city_name" => $d->city_name];
                    if($d->role == 0){
                        $role = "Admin";
                    } else {
                        $role = "User";
                ...
                <tr>
                    ...
                    <td class="text-center"><button title="Edit User" type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#add-user" onclick="editUser({{ json_encode($data) }})"><i class="fa fa-edit"></i></button> <button title="Hapus User" type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#delete-user" onclick="deleteUser({{ json_encode($data) }})"><i class="fa fa-trash"></i></button></td>
                </tr>
                @endforeach
            @else
                <tr class="text-center">
                    <td colspan="5">{{ __('No user.') }}</td>
                </tr>
            @endif
            </tbody>
            </table>
